...
落地在 `.pi/extensions/everos-memory/`：
...
```bash
pi install "$PWD/.pi/extensions/everos-memory"   # 写入用户级 ~/.pi/agent/settings.json，全机生效
# 或 pi install -l ./.pi/extensions/everos-memory # 写入项目级 .pi/settings.json，随仓库共享
```
